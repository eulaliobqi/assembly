...
Da tabela integrada (`results/annotation_complete.tsv`, 12.445 proteínas), 62,1% possuem domínio Pfam, 44,0% possuem termo GO e 42,6% possuem ortólogo KEGG (eggNOG-mapper). Uma coluna CAZy foi adicionada (1,4% das proteínas, 175 no total), mas **nenhum hit às famílias GH28/GH5** (celulases/pectinases classicamente associadas a efetores fitotóxicos de degradação de parede celular) foi encontrado — resultado consistente com a baixa sensibilidade conhecida da anotação CAZy do eggNOG, motivando uma análise dbCAN dedicada (Seção 2.6).
...
2. Cobertura de anotação por eggNOG/KEGG (~43-44%) e a ausência de hits CAZy GH28/GH5 podem refletir tanto ausência real dessas atividades enzimáticas quanto limitações de sensibilidade das ferramentas usadas — não são equivalentes.
